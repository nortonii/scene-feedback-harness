...
カメラ情報のある参照画像を選ぶと、3D ビューはその撮影位置に自動で切り替わります。手動で回転した後は `对齐参考视角`（「参照視点に合わせる」）を押すと元のカメラに戻れます。重ね合わせ表示で輪郭を比較できます。歪み補正済みの画像がある場合は、その画像をピンホール投影に合わせた重ね合わせに使います。元画像は表示とフィードバック用に別途保持します。カメラ情報のない画像は従来どおり手動で比較できます。内部パラメータが近似値の場合は、画像の端にずれが残ることがあります。
