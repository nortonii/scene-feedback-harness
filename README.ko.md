...
카메라 정보가 있는 참고 이미지를 선택하면 3D 뷰가 해당 촬영 시점으로 자동 이동합니다. 오버레이는 기본으로 표시되며 투명도를 바로 조절할 수 있습니다(0%로 설정하면 숨김). 수동으로 회전한 뒤에는 `对齐参考视角`(「참고 시점에 정렬」)을 눌러 다시 맞출 수 있습니다. 왜곡 보정 이미지가 제공되면 핀홀 카메라 투영과 맞는 오버레이에 사용합니다. 원본 참고 이미지는 보기와 피드백을 위해 별도로 보관됩니다. 카메라 정보가 없는 이미지는 계속 수동으로 비교할 수 있습니다. 내부 파라미터가 근삿값이면 이미지 가장자리에 오차가 남을 수 있습니다.
...
아직 전송되지 않은 것으로 확인된 피드백은 연결할 수 없거나 원래 작업이 실행 중일 때 대기열에 남으며, 조건이 회복되면 자동으로 재시도합니다. 전송이 확실히 실패한 경우에는 페이지에서 수동으로 재시도할 수 있습니다. 전달 여부가 불확실하면 작업대가 피드백 ID로 원래 작업 기록을 확인합니다. 여전히 확인되지 않은 항목은 새 피드백을 막지 않도록 격리하지만 **자동으로 다시 보내지 않습니다**. 원래 작업에 해당 피드백이 없는 것을 확인한 뒤 재시도를 선택하세요.

담당 agent를 바꾸려면 새로 맡을 Codex Desktop 대화를 열고 작업대의 「피드백을 받을 Codex 작업」에서 선택하세요. 현재 장면과 참고 이미지는 유지되고, 이후의 새 피드백은 선택한 작업으로 전송됩니다. 아직 보내지 않은 이전 피드백은 원래 작업에 연결된 채로 남아 그 작업으로 다시 전환했을 때만 전송됩니다. 모델은 Codex 작업 자체에서 정하며 작업대의 대상만 바꿔도 모델은 바뀌지 않습니다.
