...
Отзывы, которые заведомо ещё не отправлены, остаются в очереди, пока соединение недоступно или исходная задача занята, и автоматически повторяются после восстановления. Если отправка определённо не удалась, её можно повторить вручную на странице. Когда результат доставки неизвестен, рабочая область сверяет историю исходной задачи по ID отзыва. Неопределённая запись изолируется, чтобы последующие отзывы могли отправляться, но **автоматически повторно не отправляется**. Сначала убедитесь, что отзыва нет в исходной задаче, и только затем выберите повторную отправку.

Чтобы сменить agent, откройте задачу, которая должна продолжить работу, в Codex Desktop и выберите её в списке «Задача Codex для обратной связи» на странице. Текущая сцена и исходные изображения сохраняются. Новые отзывы поступают в выбранную задачу; ещё не отправленные отзывы остаются привязанными к прежней и продолжат отправляться только после возврата к ней. Модель задаётся самой задачей Codex: смена адресата в рабочей области её не меняет.
